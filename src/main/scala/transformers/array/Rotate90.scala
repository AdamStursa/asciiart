...
class Rotate90[T : ClassTag]() extends TwoDArrayTransformer[T] {
  ...
  override def transform(toRotate: Array[Array[T]]): Array[Array[T]] = {
    ...
    toRotate.foreach(el => require(el.length == width, "Array 2nd dimensions must all have same length"))
...
object Rotate90 {
  ...
  def apply[T: ClassTag](toRotate: Array[Array[T]]): Array[Array[T]] = new Rotate90().transform(toRotate)
}