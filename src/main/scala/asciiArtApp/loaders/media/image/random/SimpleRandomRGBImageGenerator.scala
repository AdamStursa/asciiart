...
class SimpleRandomRGBImageGenerator(val source: Int = 0) extends ImageLoader[Int, RGBImage]{
  ...
  override def load(): RGBImage = {
    val rand = new Random(source)
    val width = rand.between(MIN_SIZE, MAX_SIZE)
    val height = rand.between(MIN_SIZE, MAX_SIZE)

    val rgbImage = RGBImage(width, height)

    for (i <- 0 until height)
      for (j <- 0 until width)
        rgbImage.setPixel(i, j, new RGBPixel(rand.between(MIN_RGB, MAX_RGB), rand.between(MIN_RGB, MAX_RGB), rand.between(MIN_RGB, MAX_RGB)))
