...
import asciiArtApp.transformers.media.image.filters.grayscale.specific.GrayscaleImageInvert
import asciiArtApp.transformers.media.image.filters.grayscale.specific.flip.{GrayscaleImageFlipX, GrayscaleImageFlipY}
import asciiArtApp.transformers.media.image.filters.grayscale.specific.rotate.{GrayscaleImageRotate180, GrayscaleImageRotate270, GrayscaleImageRotate90}
...
class TempConsoleHandler {
  private var loader: Option[MediaLoader[_, _]] = None
    ...
    while (argsIter.hasNext) {
      argsIter.next() match {
        case "--image-random" =>
          ...
          loader = Option(new SimpleRandomRGBImageGenerator())
        ...
        case "--custom-table" =>
          require(convertor.isEmpty, "Only one conversion method can be defined")
          require(argsIter.hasNext, "Argument custom-table has to have value specified")
        ...
        case "--rotate" =>
          ...
          degrees match {
            case ninetyClockwise() => grayscaleFilters = grayscaleFilters.appended(new GrayscaleImageRotate90)
            case oneEighty() => grayscaleFilters = grayscaleFilters.appended(new GrayscaleImageRotate180)
            case twoSeventyClockwise() => grayscaleFilters = grayscaleFilters.appended(new GrayscaleImageRotate270)
            case _ => throw new AssertionError("Rotate value has to be multiple of 90 between -270 and (+)270")
          }
        ...
        case "--flip" =>
          ...
          axis match {
            case "x" => grayscaleFilters = grayscaleFilters.appended(new GrayscaleImageFlipX)
            case "y" => grayscaleFilters = grayscaleFilters.appended(new GrayscaleImageFlipY)
            case _ => throw new AssertionError("Flip value must be ether x or y")
          }

        case "--invert" => grayscaleFilters = grayscaleFilters.appended(new GrayscaleImageInvert)

        case "--output-console" => exporters = exporters.appended(new StdOutputExporter)

        case "--output-file" =>
          require(argsIter.hasNext, "Argument output-file has to have value specified")
          val path: String = argsIter.next()
  ...
  def execute(): Unit = {
    ...
    if (grayscaleFilters.nonEmpty) {
      val filters: GrayscaleMixedFilter = new GrayscaleMixedFilter(grayscaleFilters)
