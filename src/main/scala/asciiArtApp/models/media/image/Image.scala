...
abstract class Image[T: ClassTag](val width: Int, val height: Int)
    extends Media {
  ...
  def getPixel(x: Int, y: Int): T = {
    ...
    pixels(x)(y).ensuring(
      p => p != null,
      f"Pixel value on coordinates ($x, $y) is not set")
  }

  def getAllPixels: Array[Array[T]] =
    pixels.ensuring(
      pixels.forall(_.forall(_ != null)),
      "Some pixel value has not been set")

  def setAllPixels(input: Array[Array[T]]): Unit = {
    require(
      input.length == height,
      "Input array must have same height as image")
    input foreach (inner =>
      require(
        inner.length == width,
        "Input array must have same width as image"))
