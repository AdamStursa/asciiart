...
class CustomTableArgumentHandler(controller: Controller) extends EqualityArgumentHandler("--custom-table"){
  ...
  override def processArgument(argsIterator: Iterator[String]): Unit = {
    if (!argsIterator.hasNext)
      ...
      controller.setASCIITransformer(new LinearTableTransformer(characters.toCharArray))
    }
  }
}
